...
valid or not so
to do this task i have made these function there are different function
like which checks the different parameters and return a bool value means true or false */


/*here i have made a is char which return a boolean value true or false
...
bool isvalidemail(char *s)
{
    ...
    so i check this by using my function is char */


    if(!ischar(s[0]))
    {
        printf("\n\n\t First character of the given mail must start with an alphabet.");
        return 0;
    }
    ...
    if(at>pt || at==-1 || pt==-1 || atcount>1)
    {
        ...
        printf("\n\t You can't write '@' twice in your mail.");
...
bool ispassword(char *s)
{
    ...
    // if any of the condition fails then it wil return 0
...
// hope u will understand easily it is very straight forward
...
int signupUser(char email[], char pass[]){
    ...
    if(!ispassword(pass))
    {
        ...
        // printf("\n\n\t The password must have contain at least 8 characters.");
...
// and mind this i made this insensitive to the case
// means it allows to search with substring
